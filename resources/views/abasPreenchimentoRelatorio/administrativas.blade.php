...
@section('abaPreenchimento')
    ...
    <form method="post" id="formAdministrativas" class="col s12" action="#">
        @csrf
            ...
            <div class="row">
                <div class="col s6 left-align">
                    <button class="btn waves-effect waves-light amber darken-3" type="submit" name="action"
                            form="formAdministrativas" formaction="{{ route('preencherRelatorio', 4) }}">Anterior
                        <i class="material-icons left">arrow_back</i>
                    </button>
                </div>
                <div class="col s6 right-align">
                    <button class="btn waves-effect waves-light amber darken-3" type="submit" name="action"
                            form="formAdministrativas" formaction="{{ route('preencherRelatorio', 6) }}">Próxima
    ...
    <script>
        ...
        $("#opAulas a").on("click", function () {
            $("#formAdministrativas").attr('action', '{{ route('preencherRelatorio', 1) }}').submit();
        });

        $("#opEnsino a").on("click", function () {
            $("#formAdministrativas").attr('action', '{{ route('preencherRelatorio', 2) }}').submit();
        });

        $("#opPesquisa a").on("click", function () {
            $("#formAdministrativas").attr('action', '{{ route('preencherRelatorio', 3) }}').submit();
        });

        $("#opExtensao a").on("click", function () {
            $("#formAdministrativas").attr('action', '{{ route('preencherRelatorio', 4) }}').submit();
        });

        $("#opAdministrativas a").on("click", function () {
            $("#formAdministrativas").attr('action', '{{ route('preencherRelatorio', 5) }}').submit();
        });

        $("#opGeral a").on("click", function () {
            $("#formAdministrativas").attr('action', '{{ route('preencherRelatorio', 6) }}').submit();
        });
